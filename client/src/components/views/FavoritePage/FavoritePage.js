...
import { CloseCircleTwoTone, PlusSquareTwoTone, RightSquareTwoTone } from '@ant-design/icons';
    ...
    const renderCards = Favorites.map((favorite, index) => {
        ...
        return <tr key={index}>
            <Popover content={content} title={`${favorite.movieTitle}`}>
                <a href={`/movie/${favorite.movieId}`} style={{color:"#566270"}}><td>{favorite.movieTitle}</td></a>
            </Popover>
            <td>
                {/* 작성된 리뷰가 없다면 '새로 작성' 활성화 */}
                {(favorite.detailed.length == 0 &&
                    <Link to={{pathname: `/review/submit/${favorite.movieId}`, state:{movieInfo: favorite.detailedMovie[0]}}}> 
                        <PlusSquareTwoTone twoToneColor="#52c41a" style={{fontSize:'20px'}}/>
                    </Link>
                )} 
                ...
                {(favorite.detailed.length !== 0 &&
                    <Link to={{pathname: `/review/${favorite.detailed[0]._id}`, 
                        ...
                        <RightSquareTwoTone twoToneColor="#7200da" style={{fontSize:'20px'}}/>
                    </Link>
                )}
            </td>
